...
def main():
    ...
    ap.add_argument("--model", required=True, help="Файл весов DQN (.pth)")
    ...
    for f in files:
        ...
        if sub.empty:
            print(f"[SKIP] {f.name}: нет валидных solutions длиной 81")
            continue
    ...
    if rows:
        ...
        print("[ERROR] Не удалось собрать ни одной метрики (проверь CSV и solutions).")
        sys.exit(3)
