# sudoku_dqn_strict_eval.py
r"""
STRICT eval for baseline DQN on Sudoku (honest version):
- Требует CSV с 'quizzes' и 'solutions' (оба 81 символов).
- "Solved" <=> итоговая сетка == solution (покомпонентно).
- Мы НЕ используем env.legal_actions(); кандидаты считаем сами по правилам.

Пример:
  python sudoku_dqn_strict_eval.py ^
    --backend sudoku_rl_patched ^
    --model C:\path\to\sudoku_dqn_final_10empty.pth ^
    --csv C:\path\to\sudoku_test_h20.csv ^
    --test-n 300
"""
...
def evaluate_with_gt(backend, agent, data: List[Tuple[np.ndarray, np.ndarray]], max_steps=81):
...
def main():
    ...
    rows = df.sample(min(args.test_n, len(df)), random_state=args.seed)
    if ("quizzes" not in rows.columns or "solutions" not in rows.columns
        or not rows["solutions"].notna().all()
        or not rows["solutions"].astype(str).str.len().eq(81).all()):
        raise ValueError("Strict eval: CSV должен содержать валидные 'quizzes' и 'solutions' (оба 81 символ).")
